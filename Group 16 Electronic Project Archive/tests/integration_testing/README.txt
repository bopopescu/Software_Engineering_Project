...
To run integration test, simply cd to code/Proximity-Angular folder on terminal and type "ng e2e", and the program will automatically run integration test and give you back the result of which test failed and which is pass in the console.

*****As of right now, the integration tests still is not be able to run probably and it may or may not print anything to the console. We will take this as a feedback for improving future works.
